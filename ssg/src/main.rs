...
fn main()
{
    ...
    let html = generate_page(&PathBuf::from("docs/foo.heml"), &table, &mut cache);
    fs::write(PathBuf::from("./foo.html"), html).expect("yikes");
}
...
fn parse(heml: String, table: &EagTable, cache: &mut FileCache) -> Result<String, String>
{
    ...
    println!("{}", &heml[(open_close + 2)..toml_end]);
    ...
    let next = replace(&eag_doc, &rlz);

    parse(next, table, cache)
}
